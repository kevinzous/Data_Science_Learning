...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ###
        ### See the following docs, as yo u may need to use some of the following functions in your implementation:
        ...
        X = pack_padded_sequence(input = X,lengths = source_lengths,enforce_sorted = True)
        #print(X.data.shape)  # 62,e  ## flattened seq 62=(sum(source_lengths))
       
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, lens_unpacked = pad_packed_sequence(sequence= enc_hiddens,batch_first = True, padding_value= 0,total_length = max(source_lengths))
        # => (b,src_len, h*2)
        
        init_decoder_hidden = torch.cat((last_hidden[0], last_hidden[1]),1)     # (2, b, h) => (b, 2h)
        init_decoder_hidden = self.h_projection(init_decoder_hidden)
        init_decoder_cell = torch.cat((last_cell[0], last_cell[1]),1) # (2, b, h) => (b, 2h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = torch.zeros((tgt_len, batch_size, self.model_embeddings.embed_size)) #  (tgt_len, b, e)
        for Y_t in torch.split(Y, tgt_len, dim = 0)[0]:
            Y_t = torch.squeeze(Y_t, dim = 0)   # (1, b, e) => (b, e)
            Ybar_t = torch.cat((Y_t, o_prev), dim = 1)  # (b, e) + (b, h) => (b, e+h)
            dec_state, o_t, e_t  = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj,enc_masks)  # (b, e+h)
            combined_outputs.append(o_t)
            o_prev = o_t
        combined_outputs = torch.stack(combined_outputs,dim = 0) # List[(b, h)] of length tgt_len =>  (tgt_len, b, h) 
        ### END YOUR CODE
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t =  torch.squeeze(torch.bmm(enc_hiddens_proj,torch.unsqueeze(dec_hidden,dim=2)), dim = 2) #  (b, src_len, h) & (b,h)  => (b, src_len) 
        ...
        ### END YOUR CODE
        sm = nn.Softmax()
        alpha_t = sm(e_t) # (b, src_len) => (b, src_len)
        a_t = torch.squeeze( torch.bmm( torch.unsqueeze(alpha_t, dim = 1), enc_hiddens), dim = 1)  # (b, src_len) & (b, src_len, 2h) =>  (b, 2h)
        U_t = torch.cat((dec_hidden, a_t),dim = 1)  # (b, h) & (b, 2h) => (b, 3h)
