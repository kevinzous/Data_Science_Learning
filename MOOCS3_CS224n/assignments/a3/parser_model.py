...
+
        ...
        for b_i,batch in enumerate(w) :
            for w_i,word in enumerate(batch) :
                b_i][w_i*self.embed_size:(w_i+1)*self.embed_size] = self.embeddings[word]
        '''     
    ...
    def forward(self, w):
        ...
        hidden_droppout= self.dropout(hidden)
        logits = torch.matmul(hidden_droppout, self.hidden_to_logits_weight) + self.hidden_to_logits_bias 
