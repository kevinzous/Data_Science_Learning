...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses :
        ...
        while minibatch: 
            ...
            for i,(p,t) in enumerate(zip(minibatch,transitions)):
                p.parse_step(t)     ## or p.parse([t]) transition should be a list 
                minibatch = [p for p in minibatch if len(p.stack) == 1 and len(p.buffer) ==0]
